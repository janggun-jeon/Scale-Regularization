...
parser.add_argument('--data', type=str, default='amazon', choices=['amazon', 'yelp'], help='The dataset name. [yelp, amazon, mimic]')
...
parser.add_argument('--batch_size', type=int, default=1024, help='Batch size 1024 for yelp, 256 for amazon, X for mimic.')
...
parser.add_argument('--embed_rescalied', default=False, action='store_true', help='whethier or not embed-nodes rescalied.')
parser.add_argument('--scale_regularization', type=str, default='L1_norm', choices=['L1_norm', 'L2_norm', 'batch_norm', 'layer_norm'])

# other args
parser.add_argument('--use_cuda', default=True, action='store_true', help='Training with CUDA.')
...
if __name__ == '__main__':
    ...
    # generate log folder
    log_save_path = args.log_path + args.data + '/'
    if not os.path.exists(log_save_path): os.mkdir(log_save_path)
    print(f"Log save path: ./{log_save_path}", flush=True)
    ...
    if args.cuda: features.cuda()
    
    """
    initialize embedding dim
    """
    ...
    print('Model:  {0}, Inter-AGG:  {1}, feat_size:  {2}, emb_size:  {3}.'.format(args.model, args.inter, feat_data.shape[1], emb_size))
    if args.model == 'RIO':
        adj_lists = relations
        ...
        inter1 = InterAgg(width_rl, height_rl, args.device, args.LR, args.GAMMA, args.stop_num,
                          ...
                          cuda=args.cuda, embed_rescalied=args.embed_rescalied, regularizer=args.scale_regularization)
        gnn_model = OneLayerRio(2, inter1, args.lambda_1)
    elif args.model == 'SAGE':
        adj_lists = homo
        agg1 = MeanAggregator(features, cuda=args.cuda)
        enc1 = Encoder(features, feat_data.shape[1], emb_size, adj_lists, agg1, gcn=True, cuda=args.cuda)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in range(num_batches):
            ...
            optimizer.zero_grad()
            if args.cuda:
                batch_loss, embed = gnn_model.loss(batch_nodes, Variable(torch.cuda.LongTensor(batch_label)))
                # batch_loss = gnn_model.loss(batch_nodes, Variable(torch.cuda.LongTensor(batch_label)))
                embed_dist = np.concatenate([embed_dist, embed.detach().cpu().numpy().reshape(-1)])
            else:
        ...
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~', flush=True)
        print('Loss: {0}, time: {1}s'.format(loss / num_batches, epoch_time), flush=True)
        
        embed_log = np.concatenate([embed_log, embed_dist.reshape(1,-1)], axis=0)
    ...
    sr = '-sr' if args.embed_rescalied else ''
    # np.save(f'./RioGNN{sr}-emb_dims-{emb_size}.npy', embed_log)
      
    # make logdir
    if args.embed_rescalied:
        log_save_path = log_save_path + 'seed-' + str(args.seed) + '/'
        if not os.path.exists(log_save_path): os.mkdir(log_save_path)
        if not os.path.exists(log_save_path + 'log/'): os.mkdir(log_save_path + 'log/')
    else:
        log_save_path = log_save_path + 'old-' + str(args.seed) + '/'
        if not os.path.exists(log_save_path): os.mkdir(log_save_path)
        if not os.path.exists(log_save_path + 'log/'):  os.mkdir(log_save_path + 'log/')
        
    # save log    
    with open(log_save_path + 'gnn_auc.txt', 'a') as file:
        file.writelines(str(gnn_auc) + '\n')
    with open(log_save_path + 'gnn_recall.txt', 'a') as file:
        file.writelines(str(gnn_recall) + '\n')
