...
class InterAgg(nn.Module):
    ...
    def __init__(self, width_rl, height_rl, device, LR, GAMMA, stop_num,
                 ...
                 inter, cuda=True, embed_rescalied=False):
        """
    ...
    def forward(self, nodes, labels, train_flag=True):
        ...
        if self.embed_rescalied:
            embed0 = torch.sum(center_nodes, dim=0, keepdim=True)
            embed1 = torch.sum(combined.t(), dim=0, keepdim=True)
            embed0 = torch.sum(embed0)
            embed1 = torch.sum(embed1)

            embed  = embed0 / embed1
            combined  = (embed * combined.t()).t()
