...
class InterAgg(nn.Module):
    ...
    def __init__(self, width_rl, height_rl, device, LR, GAMMA, stop_num,
                 ...
                 inter, cuda=True, embed_rescalied=False, regularizer='L1_norm'):
        """
        ...
        self.inter = inter
        self.cuda = cuda
        self.embed_rescalied = embed_rescalied
    ...
    def forward(self, nodes, labels, train_flag=True):
        ...
        center_nodes  = self.features(torch.cuda.LongTensor(itemgetter(*nodes)(id_mapping))).detach()
        ...
        embed = 1.0
        if self.embed_rescalied:
            if self.regularizer == 'L1_norm':
                embed0 = torch.norm(center_nodes, p=1, dim=1).sum().item()
                embed1 = torch.sum(combined.t(), dim=0, keepdim=True).sum().item()
                embed  = embed0 / embed1
            if self.regularizer == 'L2_norm': 
                embed0 = torch.norm(center_nodes, p=2, dim=1).sum().item()
                embed1 = torch.sum(combined.t(), dim=0, keepdim=True).sum().item()
                embed  = embed0 / embed1
            if self.regularizer == 'layer_norm':
                embed = 1 / torch.mean(combined, dim=0, keepdim=True)
            if self.regularizer == 'batch_norm':    
                embed = 1 / torch.mean(combined, dim=0, keepdim=True)

        return embed * combined, center_scores, torch.sum(combined, dim=0, keepdim=True)
